...
read_matrix <- function(gr, cov, reverse_reads=FALSE, df=F, nu=25) {
  if(class(cov) == "character") {
    ...
    cov <- import(cov, which=gr, as="RleList")
  } 
...
standard_metapeak_matrix <- function(regions.gr, sample.cov, upstream=100, downstream=100, keep_region_coordinates = F, diff_length = F, approx_nu=25) {
  if(diff_length == F){
...
exo_metapeak_matrix <- function(regions.gr, sample, upstream=100, downstream=100, keep_region_coordinates = F) {
  if(keep_region_coordinates){
    reads <- matrix(nrow=length(regions.gr), ncol=width(regions.gr)[1])
  } else {
...
get_standard_metapeak <- function(gr, sample, upstream=100, downstream=101, smooth=NA){
  sample_path = import(sample) #returns problems with getSeqlevels (try just immediately going to standard_metapeak)
