...
remove_palindromic_motifs_from_bpnet_instances<-function(dfi, motif){

  if(class(dfi)[1]=="GRanges"){
    dfi.gr<-dfi
  } else{
    #Convert to GRanges
    dfi.gr<-makeGRangesFromDataFrame(dfi, keep.extra.columns = T, starts.in.df.are.0based = T,
                                     seqnames.field = 'example_chrom', start.field = 'pattern_start_abs', end.field = 'pattern_end_abs')
  }

  #Subset by motifs
  dfi_filt.gr<-dfi.gr %>% plyranges::filter(pattern_name==motif)
  ...
  dfi_unique.gr <- dfi_filt.gr %>% as.data.frame %>%
    dplyr::group_by(island_idx, example_idx) %>% #Group by both island_id and example_idx to keep the same motifs mapped across different task-annotated windows
    dplyr::slice_max(order_by = contrib_weighted, n = 1) %>% as.data.frame() %>%
